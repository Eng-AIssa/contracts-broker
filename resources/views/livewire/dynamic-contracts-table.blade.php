<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="md:flex md:items-center md:justify-between mb-5 mx-1">
            <div
                class="flex flex-wrap mr-1 items-center overflow-hidden shadow-md bg-white border divide-x rounded-lg rtl:flex-row-reverse">
                <a wire:click="$set('status', '%')">
                    <button
                        class="px-5 py-2 text-xs font-medium text-gray-600 transition-colors duration-200 sm:text-sm hover:bg-gray-100
                            {{$status == '%' ? 'bg-indigo-200' : ''}}">
                        {{ __("View All") }}{{ ' - ' . $statuses->count}}
                    </button>
                </a>
                @foreach($contract_statuses as $item)
                    <a wire:click="$set('status', '{{$item['ar']}}')">
                        <button
                            class="px-5 py-2 text-xs font-medium text-gray-600 transition-colors duration-200 sm:text-sm hover:bg-gray-100
                            {{($status == $item['ar']) ? 'bg-indigo-200' : ''}}">
                            {{ $item['en'] }}
                            {{($status == $item['ar']) ? $this->statusCount($item['en']) : ''}}
                        </button>
                    </a>
                @endforeach
            </div>
            ...
            <div class="relative flex items-center shadow-md mt-4 md:mt-0">
                ...
                <input wire:model="search" type="text" placeholder="{{__('Search, id | unit | owner | entry date')}}"
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex justify-end gap-4">
                                <a href="{{route('showFile', $contract->id)}}" target=”_blank”>
